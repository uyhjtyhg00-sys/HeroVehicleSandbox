...
foreach ($Path in @($HeroTypesPath, $HeroCharacterPath, $PlayerControllerHeaderPath, $PlayerControllerCppPath, $SettingsWidgetPath, $InputPath)) {
    ...
    $Backup = "$Path.input_sens_fix_v2.bak"
    if (!(Test-Path $Backup)) {
...
# ------------------------------------------------------------
# 1) HeroTypes.h: Overwatch yaw 0.0066 sensitivity model.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 3) PlayerController: restore game input mode and ESC settings toggle.
#    v2 avoids PowerShell -replace concatenation bug by appending methods.
# ------------------------------------------------------------
...
Write-Host "Applied input/sensitivity/settings fix v2."

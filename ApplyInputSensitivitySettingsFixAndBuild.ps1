...
foreach ($Path in @($HeroTypesPath, $HeroCharacterPath, $PlayerControllerHeaderPath, $PlayerControllerCppPath, $SettingsWidgetPath, $InputPath)) {
    ...
    $Backup = "$Path.input_sens_fix.bak"
    if (!(Test-Path $Backup)) {
...
# ------------------------------------------------------------
# 1) Overwatch-style sensitivity:
#    degrees/count = sensitivity * 0.0066
...
if ($HeroTypesNew -eq $HeroTypes) {
    throw "Failed to replace FHeroAimSettings in HeroTypes.h"
}
Write-Utf8NoBom $HeroTypesPath $HeroTypesNew

# ------------------------------------------------------------
# 2) HeroCharacter look: direct ControlRotation, bypass UE InputYawScale.
# ------------------------------------------------------------
...
{
    if (Controller && Value != 0.0f && !HealthComponent->IsDead())
    {
...
$HeroCharacterNew = [regex]::Replace($HeroCharacter, 'void AHeroCharacter::Turn\(const float Value\)\s*\{.*?\n\}', $NewTurn, [System.Text.RegularExpressions.RegexOptions]::Singleline)
$HeroCharacterNew = [regex]::Replace($HeroCharacterNew, 'void AHeroCharacter::LookUp\(const float Value\)\s*\{.*?\n\}', $NewLookUp, [System.Text.RegularExpressions.RegexOptions]::Singleline)
if ($HeroCharacterNew -eq $HeroCharacter) {
    throw "Failed to patch Turn/LookUp in HeroCharacter.cpp"
}
Write-Utf8NoBom $HeroCharacterPath $HeroCharacterNew

# ------------------------------------------------------------
# 3) PlayerController game input: make Sandbox always release UI input lock.
# ------------------------------------------------------------
...
'@
    $PCC = $PCC -replace 'void AHeroPlayerController::Tick\(const float DeltaSeconds\)\s*\{.*?\n\}', '$0' + $SetupFunc
}
...
'@
    $PCC = $PCC -replace 'void AHeroPlayerController::StartSandboxFromMenu\(\)\s*\{.*?\n\}', '$0' + $ToggleFunc
}
...
# ------------------------------------------------------------
# 4) Settings UI: spinbox + direct text input boxes.
# ------------------------------------------------------------
$SettingsWidgetCpp = @'
...
# ------------------------------------------------------------
# 5) Config: legacy input must be true legacy classes and mouse scale 1.
# ------------------------------------------------------------
...
# Use legacy input classes because the project binds legacy BindAxis/BindAction in SetupPlayerInputComponent.
...
# ------------------------------------------------------------
# 6) Carry-forward small compile fixes from prior patches.
# ------------------------------------------------------------
...
Write-Host "Applied input/sensitivity/settings fix."
Write-Host "Important:"
Write-Host "  - Runtime sensitivity = in-game sensitivity * 0.0066 degrees/count"
Write-Host "  - DPI only affects cm/360 display"
Write-Host "  - Config mouse axis scale is forced to 1.0"
Write-Host "  - Legacy input classes are restored so BindAxis/BindAction work"
Write-Host "  - Settings now has editable numeric boxes"
Write-Host ""
